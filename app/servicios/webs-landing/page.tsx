    ...
    <div className="min-h-screen bg-[#f5f7f8] font-sans selection:bg-blue-600/20 pb-20">
      ...
      <nav className="bg-white/80 backdrop-blur-md sticky top-0 z-50 border-b border-slate-100 h-16 flex items-center shadow-sm">
        <div className="max-w-7xl mx-auto px-4 w-full flex justify-between items-center">
          ...
          <span className="text-xs font-black tracking-widest text-blue-600 uppercase">Webs & Landing Pages · SFFALCON</span>
        </div>
      </nav>
      ...
      <div className="max-w-7xl mx-auto px-4 pt-10">
        {/* Compact Header */}
        <div className="mb-16">
          <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight mb-6">Tu web: un comercial trabajando 24/7 para ti</h1>
        ...
        <div className="space-y-12">
          <div className="grid md:grid-cols-1 lg:grid-cols-3 gap-8">
            {examples.map((example, index) => (
              <div 
                key={index}
                className="group bg-white rounded-3xl p-6 border border-slate-200 hover:border-blue-300 transition-all duration-300 hover:shadow-xl flex flex-col h-full"
              >
                ...
                <p className="text-slate-600 text-sm mb-6 flex-grow line-clamp-2">{example.description}</p>
                ...
                <div className="pt-4 border-t border-slate-100 italic text-[11px] text-slate-400 mb-6">
                  {example.benefit}
                </div>
                
                <Link 
                  href={example.href}
                  className="w-full py-3 px-4 bg-slate-900 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 group/btn hover:bg-slate-800 transition-all active:scale-[0.98]"
                >
                  Ver este modelo en acción
          ...
          {/* CTA Section */}
          <div className="mt-16 bg-blue-600 rounded-3xl p-10 text-center text-white relative overflow-hidden shadow-2xl">
            <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -mr-32 -mt-32 blur-3xl"></div>
            <div className="absolute bottom-0 left-0 w-64 h-64 bg-blue-800/20 rounded-full -ml-32 -mb-32 blur-3xl"></div>
            
            <h2 className="text-3xl md:text-4xl font-black mb-6 relative z-10">¿Hablamos de cómo multiplicar tus resultados?</h2>
