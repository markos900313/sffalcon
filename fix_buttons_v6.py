...
def get_button_html(side):
    # Added Media Query to hide on Desktop (>= 1024px)
    html = """
<style>
...
def fix_file(rel_path):
    ...
    content = re.sub(r'<style>\s*@media \(min-width: 1024px\).*?</style>', '', content, flags=re.DOTALL)
    ...
    with open(abs_path, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Fixed (v6 Responsive Conditional): {rel_path}")
